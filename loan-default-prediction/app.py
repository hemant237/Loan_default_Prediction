...
def add_features(df):
    ...
    df["risk_score"] = (df["num_derogatory_marks"] * 10 + df["debt_to_income_ratio"] * 5 - (df["credit_score"] - 300) / 100)
    df["career_maturity"] = df["employment_years"] / (df["age"] - 18).clip(lower=0.01)
    return df

@st.cache_resource
def load_model():
    data_path = Path("/mount/src/loan_default_prediction/loan-default-prediction/data/loan_default_dataset.csv")
    with st.spinner("Training model — please wait ~60 seconds..."):
        ...
        pre = ColumnTransformer([("num", Pipeline([("imputer", SimpleImputer(strategy="median")), ("scaler", StandardScaler())]), features)])
        model = Pipeline([("preprocessor", pre), ("classifier", LogisticRegression(C=1.0, class_weight="balanced", solver="lbfgs", max_iter=1000, random_state=42))])
        model.fit(X_train, y_train)
    return model, features

st.title("🏦 Loan Default Risk Predictor")
st.markdown("Enter applicant details below. The model returns a **default probability** and a **risk decision**.")
st.divider()
...
applicant = {"age": age, "annual_income": annual_income, "employment_years": employment_yrs, "credit_score": credit_score, "loan_amount": loan_amount, "loan_term_months": loan_term, "num_open_accounts": num_accounts, "debt_to_income_ratio": dti, "num_derogatory_marks": derog_marks}
...
if st.button("⚡ Assess Risk", type="primary"):
    ...
    with r2:
        st.error(f"🔴 Risk: **{risk}**") if risk == "HIGH" else st.success(f"🟢 Risk: **{risk}**")
    with r3:
        st.error(f"❌ **{decision}**") if decision == "DECLINE" else st.success(f"✅ **{decision}**")
    st.progress(min(proba, 1.0))
    st.caption(f"Threshold: {THRESHOLD:.0%} · Above = HIGH risk")
