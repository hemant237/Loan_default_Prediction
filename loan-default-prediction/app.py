...
# ── Feature engineering (inline — no src/ import needed) ──────────────────── #
def add_features(df):
    df = df.copy()
...
if st.button("⚡ Assess Risk", type="primary"):
    ...
    st.dataframe(pd.DataFrame({
        ...
        "Value":          [f"{fe['payment_to_income']:.3f}", f"{fe['risk_score']:.2f}",
                           f"{fe['career_maturity']:.3f}", f"${fe['loan_per_account']:,.0f}"],
        "Interpretation": ["Higher → more payment burden", "Higher → more risk",
                           "Lower → less stable employment", "Higher → concentrated loan"],
